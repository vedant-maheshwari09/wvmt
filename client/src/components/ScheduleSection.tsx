        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-3 sm:p-6 lg:p-8 shadow-xl">
          <div className="space-y-2 sm:space-y-4">
            {scheduleItems.map((item, index) => {
              ...
              return (
                <div key={index} className={`flex flex-col sm:flex-row sm:items-start space-y-2 sm:space-y-0 sm:space-x-4 p-3 sm:p-4 ${bgClass} rounded-xl sm:rounded-2xl overflow-hidden`}>
                  <div className={`${item.color} ${colorClass} px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl font-bold text-center text-xs sm:text-base sm:min-w-[140px] w-full sm:w-auto flex-shrink-0`}>
                    {item.time}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className={`font-bold text-sm sm:text-lg mb-1 sm:mb-2 break-words ${
                    ...
                    <p className="text-gray-700 text-xs sm:text-base break-words">{item.description}</p>
                  </div>
                </div>
              );
            })}
