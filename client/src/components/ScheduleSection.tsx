  ...
  return (
    <section id="schedule" className="bg-pattern-cream pattern-overlay py-16">
      <div className="content-layer max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl">
          <div className="space-y-4 sm:space-y-6">
            {scheduleItems.map((item, index) => {
              ...
              return (
                <div key={index} className={`flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-6 p-4 ${bgClass} rounded-2xl`}>
                  <div className={`${item.color} ${colorClass} px-3 py-2 rounded-xl font-bold text-center text-sm sm:text-base sm:min-w-[140px] w-full sm:w-auto`}>
                    {item.time}
                  </div>
